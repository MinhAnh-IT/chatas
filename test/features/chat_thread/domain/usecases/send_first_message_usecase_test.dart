...
void main() {
  group('SendFirstMessageUseCase', () {
    ...
    test(
      'should unhide existing thread and set lastRecreatedAt when recreating chat',
      () async {
        // Arrange
        ...
        when(
          mockChatThreadRepository.reviveThreadForUser(
            'original_thread_123',
            'userB',
          ),
        ).thenAnswer((_) async => {});
        when(
          mockChatThreadRepository.updateVisibilityCutoff(
            'original_thread_123',
            'userB',
            now,
          ),
        ...
        when(
          mockChatThreadRepository.resetUnreadCount(
            'original_thread_123',
            'userB',
          ),
        ).thenAnswer((_) async => {});
        when(
          mockChatMessageRepository.sendMessage(any),
        ).thenAnswer((_) async => {});
    ...
    test('should create new thread when temporary thread is used', () async {
      ...
      when(
        mockChatThreadRepository.createChatThread(any),
      ).thenAnswer((_) async => {});
      when(
        mockChatMessageRepository.sendMessage(any),
      ).thenAnswer((_) async => {});
      ...
      // Act
      final result = await useCase(chatThread: tempThread, message: message);
      ...
      verifyNever(
        mockChatThreadRepository.updateVisibilityCutoff(any, any, any),
      );
    });
  });
}
