...
void main() {
  group('MarkThreadDeletedUseCase', () {
    ...
    test(
      'should mark thread as deleted for user with current time as cutoff',
      () async {
        // Arrange
        ...
        when(
          mockRepository.markThreadDeletedForUser(any, any, any),
        ).thenAnswer((_) async {});

        // Act
        await useCase(threadId: threadId, userId: userId);

        // Assert
        verify(
          mockRepository.markThreadDeletedForUser(
            threadId,
            userId,
    ...
    test(
      'should mark thread as deleted with lastMessageTime as cutoff when provided',
      () async {
        // Arrange
        const threadId = 'test_thread_123';
        const userId = 'user_456';
