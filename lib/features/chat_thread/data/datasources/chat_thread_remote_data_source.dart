...
class ChatThreadRemoteDataSource {
  ...
  Future<List<ChatThreadModel>> fetchChatThreads(String currentUserId) async {
    print('ChatThreadRemoteDataSource: Fetching threads for user: $currentUserId');
    final snapshot = await firestore
        .collection(ChatThreadRemoteConstants.collectionName)
        .where('members', arrayContains: currentUserId) // Only threads where user is a member
        .get();
    print('ChatThreadRemoteDataSource: Found ${snapshot.docs.length} threads for user $currentUserId');
    
    // Debug: Print details of each thread
    final threads = snapshot.docs.map((doc) {
      final data = doc.data();
      data['id'] = doc.id; // Set document ID from Firestore
      print('ChatThreadRemoteDataSource: Thread ${doc.id} - Members: ${data['members']}, Name: ${data['name']}');
  ...
  Future<void> addChatThread(ChatThreadModel model) async {
    print('ChatThreadRemoteDataSource: Adding chat thread with ID: ${model.id}');
  ...
  Stream<List<ChatThreadModel>> chatThreadsStream(String currentUserId) {
    print('ChatThreadRemoteDataSource: Setting up threads stream for user: $currentUserId');
    return firestore
        .collection(ChatThreadRemoteConstants.collectionName)
        .where('members', arrayContains: currentUserId) // Only threads where user is a member
        .snapshots()
        .map((snapshot) {
          print('ChatThreadRemoteDataSource: Stream received ${snapshot.docs.length} threads for user');
